...
- Never use null-forgiving operator:
	...
	```
- Use nullability attributes when it aids the compiler and improves code clarity:
	```csharp
	...
	```
- Use the `required` keyword for properties with non-null validation that do not have a constructor where the properties can be set:
	```csharp
	// Good: Non-null validation in constructor
	public class AvatarModel
	{
		public required Color BackgroundColor { get; init; } = Colors.Black;
		public requiredColor BorderColor { get; init; } = Colors.White;
...
- Always dispose CancellationTokenSources:
	...
	public async Task<Order> GetOrderWithTimeout(OrderId id)
	{
		...
		return await _repository.GetAsync(id, cts.Token).ConfigureAwait(false);
	}
	```
