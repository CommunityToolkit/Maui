...
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(UseCommunityToolkitMediaViewInitializationAnalyzerCodeFixProvider)), Shared]
public class UseCommunityToolkitMediaViewInitializationAnalyzerCodeFixProvider : CodeFixProvider
{
	public sealed override ImmutableArray<string> FixableDiagnosticIds { get; } = ImmutableArray.Create(UseCommunityToolkitMediaViewInitializationAnalyzer.DiagnosticId);
	...
	public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
	{
		...
		context.RegisterCodeFix(
			CodeAction.Create(
				title: CodeFixResources.Initialize__NET_MAUI_Community_Toolkit_MediaView_Before_UseMauiApp,
				createChangedDocument: c => AddUseCommunityToolkit(context.Document, declaration, c),
				equivalenceKey: nameof(CodeFixResources.Initialize__NET_MAUI_Community_Toolkit_MediaView_Before_UseMauiApp)),
			diagnostic);
	}
		...
		var updatedInvocationExpression =
			InvocationExpression(
				MemberAccessExpression(
					SyntaxKind.SimpleMemberAccessExpression, invocationExpression, IdentifierName("UseMauiCommunityToolkitMediaView")));
