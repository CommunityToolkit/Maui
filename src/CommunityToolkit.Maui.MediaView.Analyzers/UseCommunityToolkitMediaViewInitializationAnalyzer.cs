...
namespace CommunityToolkit.Maui.MediaView.Analyzers;

[DiagnosticAnalyzer(LanguageNames.CSharp)]
public class UseCommunityToolkitMediaViewInitializationAnalyzer : DiagnosticAnalyzer
{
	...
	static bool HasUseMauiCommunityToolkit(SyntaxNode root)
	{
		...
		{
			if (method.DescendantNodes().OfType<ExpressionStatementSyntax>().Any(x => x.DescendantNodes().Any(x => x.ToString().Contains(".UseMauiCommunityToolkitMediaView("))))
			{
				return true;
			}
		}
