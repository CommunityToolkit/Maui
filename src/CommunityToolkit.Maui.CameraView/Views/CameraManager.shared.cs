...
partial class CameraManager(IMauiContext mauiContext, ICameraView cameraView, CameraProvider cameraProvider, Action onLoaded) : IDisposable
{
	CameraInfo? currentCamera = cameraView.SelectedCamera ??= cameraProvider.AvailableCameras.First() ?? throw new InvalidOperationException("No available camera found.");
