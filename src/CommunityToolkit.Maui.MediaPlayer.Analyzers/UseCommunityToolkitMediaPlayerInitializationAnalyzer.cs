...
namespace CommunityToolkit.Maui.MediaPlayer.Analyzers;

[DiagnosticAnalyzer(LanguageNames.CSharp)]
public class UseCommunityToolkitMediaPlayerInitializationAnalyzer : DiagnosticAnalyzer
{
	...
	static bool HasUseMauiCommunityToolkit(SyntaxNode root)
	{
		...
		{
			if (method.DescendantNodes().OfType<ExpressionStatementSyntax>().Any(x => x.DescendantNodes().Any(x => x.ToString().Contains(".UseMauiCommunityToolkitMediaPlayer("))))
			{
				return true;
			}
		}
