...
class TextColorToGenerator : ISourceGenerator
{
	...
	{
		var textStyleTypes = GetITextStyleTypes(Assembly.LoadFrom("Microsoft.Maui.Controls"));

		foreach (var textStyleType in textStyleTypes)
		{
...
public static partial class ColorAnimationExtensions
{
	...
	public static Task<bool> TextColorTo(this" + textStyleType.Name + @"element, Color color, uint rate = 16u, uint length = 250u, Easing? easing = null)
	{
		...
		static Animation GetRedTransformAnimation(TTextStyle element, float targetRed) =>
			new(v => element.TextColor = element.TextColor.WithRed(v), element.TextColor.Red, targetRed);

		static Animation GetGreenTransformAnimation(TTextStyle element, float targetGreen) =>
			new(v => element.TextColor = element.TextColor.WithGreen(v), element.TextColor.Green, targetGreen);

		static Animation GetBlueTransformAnimation(TTextStyle element, float targetBlue) =>
			new(v => element.TextColor = element.TextColor.WithBlue(v), element.TextColor.Blue, targetBlue);

		static Animation GetAlphaTransformAnimation(TTextStyle element, float targetAlpha) =>
			new(v => element.TextColor = element.TextColor.WithAlpha(v), element.TextColor.Alpha, targetAlpha);
	}
}";
			context.AddSource($"{textStyleType.Name}TextColorTo.g.cs", SourceText.From(textColorToBuilder.ToString(), Encoding.UTF8));
		}
	}

	void ISourceGenerator.Initialize(GeneratorInitializationContext context) => throw new NotImplementedException();

	static IEnumerable<Type> GetITextStyleTypes(in Assembly assembly)
	{
		return GetLoadableTypes(assembly).Where(typeof(ITextStyle).IsAssignableFrom);

		static IEnumerable<Type> GetLoadableTypes(in Assembly assembly)
		{
