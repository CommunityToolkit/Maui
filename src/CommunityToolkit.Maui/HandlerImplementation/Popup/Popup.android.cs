...
public partial class Popup : Element
{
	...
	void RemoveHandlerChanged()
	{
		...
		{
			if (Content?.ToPlatform(mauiContext) is AView container)
			{
				container.LayoutChange -= OnContainerLayoutChanged;
			}
		}
	}
	...
	void OnContainerLayoutChanged(object? sender, AView.LayoutChangeEventArgs e)
	{
		if (Handler?.MauiContext is IMauiContext mauiContext)
		{
	...
	static void Measure(Context context, MView target, int width, int height, bool isNanWidth, bool isNanHeight, ref bool isRoot)
	{
		...
		if (!isRoot)
		{
			isNanWidth = double.IsNaN(target.WidthRequest);
			isNanHeight = double.IsNaN(target.HeightRequest);
		}
		isRoot = false;
		...
		else if (target.Parent is Grid grid)
		{
			...
			{
				requestWidth = ((target.Width != -1 && target.WidthRequest != -1) ? (int)context.ToPixels(target.Width) : width) - paddingH;
			...
			else
			{
				...
				else
				{
					if (grid.ColumnDefinitions[grid.GetColumn(target)].Width.IsStar)
					{
						...
						requestWidth = (int)((width - (int)context.ToPixels(fixWidth) - paddingH) / totalStar * grid.ColumnDefinitions[grid.GetColumn(target)].Width.Value);
			...
			{
				requestHeight = ((target.Height != -1 && target.HeightRequest != -1) ? (int)context.ToPixels(target.Height) : height) - paddingV;
			...
			else
			{
				...
				else
				{
					if (grid.RowDefinitions[grid.GetRow(target)].Height.IsStar)
					{
						...
						requestHeight = (int)((height - (int)context.ToPixels(fixHeight) - paddingV) / totalStar * grid.RowDefinitions[grid.GetRow(target)].Height.Value);
		...
		else
		{
			var requestWidth = ((target.Width != -1 && target.WidthRequest != -1) ? (int)context.ToPixels(target.Width) : width) - paddingH;
			var requestHeight = ((target.Height != -1 && target.HeightRequest != -1) ? (int)context.ToPixels(target.Height) : height) - paddingV;
			MeasureSpecMode measureSpecModeH = !isNanWidth ? MeasureSpecMode.Exactly : isHorizontalFill ? MeasureSpecMode.Unspecified : MeasureSpecMode.AtMost;
			MeasureSpecMode measureSpecModeV = !isNanHeight ? MeasureSpecMode.Exactly : isVerticalFill ? MeasureSpecMode.Unspecified : MeasureSpecMode.AtMost;
			...
			if (platformView.MeasuredWidth > requestWidth)
			{
				measureSpecModeH = MeasureSpecMode.AtMost;
			}
			if (platformView.MeasuredHeight > requestHeight)
			{
				measureSpecModeV = MeasureSpecMode.AtMost;
			}
		...
		foreach (Element element in target.LogicalChildrenInternal)
		{
			...
			{
				Measure(context, view, width, height, isNanWidth, isNanHeight, ref isRoot);
			}
		}
	}
