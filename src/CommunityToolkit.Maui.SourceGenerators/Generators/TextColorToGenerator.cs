...
class TextColorToGenerator : IIncrementalGenerator
{
	...
	public void Initialize(IncrementalGeneratorInitializationContext context)
	{
		...
		var userGeneratedClassesProvider = context.SyntaxProvider.CreateSyntaxProvider(
			...
			static (context, cancellationToken) =>
			{
				...
				// If the ClassDeclarationSyntax doesn't implement those interfaces we just return null
	...
	static void Execution(SourceProductionContext context, TextStyleClassMetadata textStyleClassMetadata)
	{
		...
		var source =
			...
			$$"""
			  ...
			  {{textStyleClassMetadata.ClassAccessModifier}} static partial class ColorAnimationExtensions_{{textStyleClassMetadata.ClassName}}
			  {
			  	/// <summary>
	...
	{
		var namedTypeSymbols = right.ToList();
		...
		foreach (var leftItem in left)
		{
			...
			var result = namedTypeSymbols.ContainsSymbolBaseType(leftItem);
			if (!result)
			{
		...
		foreach (var rightItem in namedTypeSymbols)
		{
