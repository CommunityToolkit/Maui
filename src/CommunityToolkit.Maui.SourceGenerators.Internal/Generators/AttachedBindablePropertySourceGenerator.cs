...
[Generator]
public class AttachedBindablePropertySourceGenerator : IIncrementalGenerator
{
	...
	// Use the actual BindableProperty type: CreateAttached/CreateReadOnlyAttached are static members on BindableProperty
	const string bpFullName = "global::Microsoft.Maui.Controls.BindableProperty";
	// Keep the Generated attribute text identical to test expectations to avoid formatting diffs
	const string attachedPropertyAttribute =
		/* language=C#-test */
		//lang=csharp
		$$"""
	  // <auto-generated>
	...
	public void Initialize(IncrementalGeneratorInitializationContext context)
	{
		...
		context.RegisterPostInitializationOutput(static ctx => ctx.AddSource("AttachedBindablePropertyAttribute.g.cs", SourceText.From(attachedPropertyAttribute, Encoding.UTF8)));

		var provider = context.SyntaxProvider.ForAttributeWithMetadataName("CommunityToolkit.Maui.AttachedBindablePropertyAttribute",
	...
	static string GenerateSource(SemanticValues value)
	{
		...
		// Add using for BindableObject so generated Get/Set accessors can use the unqualified type name.
		sb.Append("using global::Microsoft.Maui.Controls;\n\n");
		...
		foreach (var info in value.BindableProperties)
		{
			...
			{
				GenerateReadOnlyAttachedBindableProperty(sb, in info, fileStaticClassName);
			}
			else
			{
				GenerateAttachedBindableProperty(sb, in info, fileStaticClassName);
			}
			...
			// After generating the property, generate the attached accessors in the expected compact form
			GenerateAttachedAccessors(sb, in info);
		}
	...
	static void GenerateReadOnlyAttachedBindableProperty(StringBuilder sb, in BindablePropertyModel info, in string fileStaticClassName)
	{
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	static void GenerateAttachedBindableProperty(StringBuilder sb, in BindablePropertyModel info, in string helperClassName)
	{
		// Sanitize the Return Type because Nullable Reference Types cannot be used in the `typeof()` operator
		var nonNullableReturnType = ConvertToNonNullableTypeSymbol(info.ReturnType);
		var sanitizedPropertyName = IsDotnetKeyword(info.PropertyName) ? string.Concat("@", info.PropertyName) : info.PropertyName;
	...
	static void GenerateProperty(StringBuilder sb, in BindablePropertyModel info, in string fileStaticClassName)
	{
		...
		sb.Append("(")
			.Append(formattedNonNullableReturnType)
			.Append(")GetValue(")
			.Append(info.BindablePropertyName)
			.Append(");\n");
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	static void GenerateAttachedAccessors(StringBuilder sb, in BindablePropertyModel info)
	{
		var sanitizedPropertyName = IsDotnetKeyword(info.PropertyName) ? string.Concat("@", info.PropertyName) : info.PropertyName;
		var formattedReturnType = GetFormattedReturnType(info.ReturnType);
		var nonNullableReturnType = ConvertToNonNullableTypeSymbol(info.ReturnType);
		...
		// Append a blank line to separate property and accessors
		sb.Append('\n');
	...
	/// For attached properties the bindable may not be an instance of the declaring type (e.g. docking property on children),
	/// so we fall back to returning the default(...) for the property's type when the cast fails.
	/// </summary>
	/// <param name="fileStaticClassStringBuilder">Helper StringBuilder used to collect helper members.</param>
	/// <param name="info">Property model.</param>
