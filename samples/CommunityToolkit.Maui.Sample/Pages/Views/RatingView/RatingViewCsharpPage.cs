...
public class RatingViewCsharpPage : BasePage<RatingViewCsharpViewModel>
{
	...
	public RatingViewCsharpPage(RatingViewCsharpViewModel viewModel) : base(viewModel)
	{
		...
		Style ByStyle = new(typeof(RatingView))
		{
			Setters =
				{
					new Setter { Property = RatingView.EmptyColorProperty, Value = Colors.Green },
					new Setter { Property = RatingView.FilledColorProperty, Value = Colors.Blue },
		...
		Content = new ScrollView
		{
			Content = new VerticalStackLayout
			{
				...
				Children =
				{
					...
					new RatingView
					{
						EmptyColor = vm.ColorPickerEmptyBackgroundTarget,
						FilledColor = vm.ColorPickerFilledBackgroundTarget,
						ItemShapeSize = 40,
						MaximumRating = 5,
						Rating = 2.7,
					...
					GetSeparator(),
					new Label { Style = Description, Text = "Read only" },
					GetSeparator(),
					new HorizontalStackLayout
					{
						Spacing = 8,
						Children =
						{
							new CheckBox { IsChecked = false }.Assign(out CheckBox checkBox).SemanticHint("Check to make read only."),
							new Label { Text = ": IsReadOnly", VerticalOptions = LayoutOptions.Center },
						}
					},
